...
{% for post in post %}
    ...
    <div id="post-cm post-comment" data-postid="{{ post['post_id'] }}" class="post-cm post-comment">
      ...
      <div class="u-dtl z-index-upper">
        ...
        {% if post['profile'] == 'df' %}
            <img lazy="true" class="vpost-post-u-media" src='assets/default.jpg'>
        {% else %}
            <img lazy="true" class="vpost-post-u-media" src="http://{{ _ipaddr }}:8000/image?uuid={{ post['uid'] }}&ppic={{ post['profile'] }}&ptp=profile">
        {% endif %}
      ...
      <div>
       {% if post['post_type'] == "image" %}
         <div class="p-media-cnt">
          <img lazy="true" id="p-media" onclick="iurl('viewpost', {pid: this.getAttribute('data-pid') })" data-pid="{{ post['post_id'] }}" src="http://{{ _ipaddr }}:8000/image?uuid={{ post['uid'] }}&ppic={{ post['post_pic'] }}&ptp=post" class="p-media">
         </div>
